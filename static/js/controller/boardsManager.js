...
async function createNewBoard(e) {
    ...
    if (user_id){
        if(checkResult === 0){
            const newBoardLock = document.querySelector(`.lock[board-title-id="${board.id}"]`);
            newBoardLock.innerHTML = `<img src="/static/img/icons8-unlock-50.png" alt="unlock" data-lock="unlock">`
            newBoardLock.addEventListener('click',updateUserBoardStatus)
        }
        else{
            const newBoardLock = document.querySelector(`.lock[board-title-id="${board.id}"]`);
            newBoardLock.innerHTML = `<img src="/static/img/lock-24.png" alt="lock" data-lock="lock">`
            newBoardLock.addEventListener('click',updateUserBoardStatus)
        }
    }
